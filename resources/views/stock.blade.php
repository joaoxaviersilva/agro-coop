{{-- 
    ...
    Este arquivo define a nova tela de Mercado & Estoque de Silos utilizando @extends and @section.
    
    COMENTÁRIO TÉCNICO: Esta view inovadora centraliza as informações financeiras de commodities 
    do agronegócio e o volume real de inventário agrícola. As barras de progresso dos silos são construídas 
    utilizando divs aninhadas com larguras percentuais dinâmicas injetadas diretamente via Blade style string, 
    dispensando o uso de scripts externos e mantendo a renderização rápida do padrão MVC.
--}}
        ...
        <div class="grid gap-6 md:grid-cols-2">
            <div class="rounded-3xl border border-farm-300 bg-farm-50 p-6 shadow-sm">
                ...
                <div class="flex justify-between text-xs text-farm-700 mb-1">
                    <span>Capacidade Máxima: {{ number_format($capacidadeMaximaNorte, 0, ',', '.') }}t</span>
                    <span class="font-bold {{ $porcentagemNorte >= 85 ? 'text-barn-700' : 'text-pasture-800' }}">
                        {{ number_format($porcentagemNorte, 1, ',', '.') }}% Ocupado
                    </span>
                </div>
                
                <div class="w-full bg-farm-200 rounded-full h-4 overflow-hidden border border-farm-300">
                    <div class="{{ $porcentagemNorte >= 85 ? 'bg-barn-600' : 'bg-pasture-700' }} h-4 rounded-full transition-all duration-500" 
                         style="width: {{ $porcentagemNorte }}%"></div>
                </div>

                @if($porcentagemNorte >= 85)
                ...
                @else
                    <p class="mt-3 text-xs text-pasture-700 font-semibold flex items-center gap-1">
                        <span>✓</span> Unidade operacional: Espaço seguro para novos descarregamentos.
                    </p>
                @endif
            </div>

            <div class="rounded-3xl border border-farm-300 bg-farm-50 p-6 shadow-sm">
                <h3 class="font-bold text-farm-900 mb-4">Ocupação Física: Silo Vertical Sul (Capacidade Geral)</h3>
                <div class="flex justify-between text-xs text-farm-700 mb-1">
                    <span>Capacidade Máxima: {{ number_format($capacidadeMaximaSul, 0, ',', '.') }} cab</span>
                    <span class="font-bold {{ $porcentagemSul >= 85 ? 'text-barn-700' : 'text-pasture-800' }}">
                        {{ number_format($porcentagemSul, 1, ',', '.') }}% Ocupado
                    </span>
                </div>

                <div class="w-full bg-farm-200 rounded-full h-4 overflow-hidden border border-farm-300">
                    <div class="{{ $porcentagemSul >= 85 ? 'bg-barn-600' : 'bg-pasture-700' }} h-4 rounded-full transition-all duration-500" 
                         style="width: {{ $porcentagemSul }}%"></div>
                </div>

                @if($porcentagemSul >= 85)
                    <p class="mt-3 text-xs text-barn-700 font-semibold flex items-center gap-1">
                        <span>⚠️</span> Alerta técnico: Limite crítico de alojamento atingido.
                    </p>
                @else
                    <p class="mt-3 text-xs text-pasture-700 font-semibold flex items-center gap-1">
                        <span>✓</span> Unidade operacional: Liberado para recepção de novos lotes de pecuária.
                    </p>
                @endif
            </div>
        </div>
